...
class ClassService {
  ...
  Stream<List<CourseClass>> streamClassesForUser({
    ...
    return _db.collection('users').doc(uid).snapshots().asyncExpand((userSnap) async* {
      if (!userSnap.exists) {
        // Ghost-account recovery happens after Firebase Auth succeeds but before
        // the user's Firestore profile exists. Active classes are intentionally
        // readable by signed-in users for this setup flow.
        yield* activeClasses();
        return;
      }
